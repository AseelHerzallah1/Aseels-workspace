...
# SSE comment padding — forces Render/nginx to open the stream immediately.
_SSE_PAD = b": " + b" " * 4096 + b"\n\n"
...
async def _sse_chat_events(req: ChatRequest) -> AsyncIterator[bytes]:
    """Server-Sent Events stream — reliable token delivery through reverse proxies."""
    yield _SSE_PAD
    ...
    token_iter = chat.stream_groq(prep["system_prompt"], messages, prep["temperature"])
